...
```{r setup}
knitr::opts_knit$set(root.dir = normalizePath("C:/Users/josep/Desktop/QBIO490/qbio_490_joseph_kim/")) 
```
...
```{r}
setwd("C:/Users/josep/Desktop/QBIO490/qbio_490_joseph_kim/")
...
```{r}
write.csv(clinic, "C:/Users/josep/Desktop/QBIO490/qbio_490_joseph_kim/analysis_data/skcm_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("C:/Users/josep/Desktop/QBIO490/qbio_490_joseph_kim/analysis_data/skcm_clinical_data.csv")
```
...
as.integer(clinic$age_at_diagnosis[2]) + (as.integer(clinic$last_contact_days_to[2]) / 365)
```
...
```{r Exercise 4.1}
# create plot here
plot(x = clinic$weight_kg_at_diagnosis, y = clinic$breslow_thickness_at_diagnosis)
```
...
```{r Exercise 4.2}
# create plot here
b <- boxplot(as.integer(clinic$height_cm_at_diagnosis) ~ clinic$race)
```
...
```{r}
vital_status_na_mask <- clinic$vital_status[clinic$vital_status != '[Not Available]']
vital_status_na_mask
```

2) Create a mask to select for male patients
```{r}
male_mask <- clinic$gender[clinic$gender == 'MALE']
male_mask
```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
#clinic$age_at_diagnosis <- clinic$age_at_diagnosis[clinic$age_at_diagnosis != '[Not Available]']
clinic$age_at_diagnosis <- as.integer(clinic$age_at_diagnosis)
...
```{r}
cleaned_clinic$age_category <- ifelse(cleaned_clinic$age_at_diagnosis >= 50, "Old", ifelse(cleaned_clinic$age_at_diagnosis <= 35, "Young", "Middle"))
```
...
```{r}
cleaned_clinic$last_contact_days_to <- as.integer(cleaned_clinic$last_contact_days_to)
cleaned_clinic$death_days_to <- as.integer(cleaned_clinic$death_days_to)
...
The p-value is < 0.0001, which is statistically significant for all significance thresholds. This tells us that there exists a relationship between age and skin cancer survival.
